...
export const DEBUG_SUPPORTED_MODELS = {
  ...
  dashscope: [{ label: 'qwen-plus', value: 'qwen-plus' }]
}
...
export const isModelSupportTypes = (model, provider, inputType, returnType) => {
  return (
    isModelSupportInputType(model, provider, inputType) &&
...
export const getModelsByProviderAndInputType = (provider, inputType) => {
  ...
  // 过滤出支持该输入类型的模型
  return models.filter(m => m.capabilities?.inputTypes?.includes(inputType))
}
...
export const getModelsByProviderAndReturnType = (provider, returnType) => {
  ...
  // 过滤出支持该返回类型的模型
  return models.filter(m => m.capabilities?.returnTypes?.includes(returnType))
}
